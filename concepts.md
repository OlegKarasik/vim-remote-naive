...
4. **Command model:** `RemoteConfig` ensures and opens config; `RemoteSwitch` picks and writes `current`; `RemotePull` pulls updates from the current remote via async `rsync` in a terminal; `RemoteCancel` cancels active pull jobs.
